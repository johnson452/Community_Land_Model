...
def solar_fluxes(State, Grid, App, i):
    ...
    if State.radiation.vegetated_surface:
        ...
        tot_sol_rad_abs_g = (
            ...
            S_dif_vis * State.I_down_lambda_vis[i]) * (1 - State.a_g_lambda_vis[i]) + 
            (S_dir_nir * State.I_down_lambda_nir_mu[i] +
            S_dif_nir * State.I_down_lambda_nir[i]) * (1 - State.a_g_lambda_vis[i])
        )
    else: 
...
def r_vis_calc(total_rad_i: float, vis_frac: float) -> float:
    ...
    a_3 = 8.1351 * 10 ** (-9)
    R_vis = (
        a_0 + a_1 * (vis_frac * total_rad_i) + a_2 * (vis_frac * total_rad_i)**2 +
...
def r_nir_calc(total_rad_i: float, vis_frac: float) -> float:
    ...
    b_3 = 1.4881 * 10 ** (-8)
    R_nir = (
        b_0 + b_1 * (nir_frac * total_rad_i) + b_2 * (nir_frac * total_rad_i)**2 +
